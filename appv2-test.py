...
def load_model(model_name):
    global model
    if model is None or model.model.names[0] != model_name:
...
with gr.Blocks() as demo:
    ...
    model_dropdown = gr.Dropdown(choices=list_models(), value=available_models[0], label="Select Model")
    ...
    image_button.click(fn=process_image, inputs=[image_input, conf_slider, iou_slider, model_dropdown], outputs=[image_output, image_description])
    video_button.click(fn=process_video, inputs=[video_input, video_conf_slider, video_iou_slider, speed_slider, model_dropdown], outputs=video_output)
    realtime_button.click(fn=realtime_detection, inputs=[realtime_conf_slider, realtime_iou_slider, model_dropdown, input_source], outputs=realtime_output)
    youtube_button.click(fn=process_youtube_video, inputs=[youtube_url, video_conf_slider, video_iou_slider, speed_slider])
